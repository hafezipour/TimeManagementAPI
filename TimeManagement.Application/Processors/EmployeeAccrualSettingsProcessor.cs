...
public class EmployeeAccrualSettingsProcessor : BaseProcessor
{
    ...
    private async Task<string> GetAccrualBanks(GetAccrualBanksRequest? request)
    {
        try
        {
            ...
            // Step 1: Get EmployeeAccrualSettings by UserId (using existing Get method)
            var getSettingsRequest = new GetEmployeeAccrualSettingsRequest { UserId = request.UserId };
            var settingsJson = await Get(getSettingsRequest);
            ...
            var result = await _employeeAccrualSettingsRepository.GetAccrualBanks(
                userId, 
